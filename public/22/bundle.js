...
var Noise = _noisejs2.default.Noise;

var POINTS = 20;

var Experiment = function (_CanvasApp) {
  _inherits(Experiment, _CanvasApp);
  ...
  _createClass(Experiment, [{
    ...
    value: function update(t, delta) {
      ...
      var change = t * 0.00002;
      var mod = 2;
      for (var i = 0; i < 8; i++) {
        // const noise = this.noise.perlin2(change + 10*i, change + 4*i);
        var noise = this.noise.perlin2(5 * change + i / 8, change + i * 2);
        this.values[i] = (noise * 0.5 + 0.5) * (i % 2 ? window.innerHeight * 2 : window.innerWidth * 2) - (i % 2 ? window.innerHeight / 2 : window.innerWidth / 2);
      }

      this.ctx.beginPath();
      this.ctx.moveTo(this.values[0], this.values[1]);
      this.ctx.bezierCurveTo(this.values[6], this.values[7], this.values[4], this.values[5], this.values[2], this.values[3]);
      this.ctx.stroke();
    }
  }]);
