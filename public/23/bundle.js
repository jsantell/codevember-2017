...
module.exports = "#define GLSLIFY 1\nuniform float time;\n\nvarying vec2 vUv;\nvarying float vNoise;\nvarying vec3 vNormal;\nvarying float vR;\n\nfloat when_lt_2_0(float x, float y) {\n  return max(sign(y - x), 0.0);\n}\n\nvec2 when_lt_2_0(vec2 x, vec2 y) {\n  return max(sign(y - x), 0.0);\n}\n\nvec3 when_lt_2_0(vec3 x, vec3 y) {\n  return max(sign(y - x), 0.0);\n}\n\nvec4 when_lt_2_0(vec4 x, vec4 y) {\n  return max(sign(y - x), 0.0);\n}\n\n\n\nfloat when_gt_1_1(float x, float y) {\n  return max(sign(x - y), 0.0);\n}\n\nvec2 when_gt_1_1(vec2 x, vec2 y) {\n  return max(sign(x - y), 0.0);\n}\n\nvec3 when_gt_1_1(vec3 x, vec3 y) {\n  return max(sign(x - y), 0.0);\n}\n\nvec4 when_gt_1_1(vec4 x, vec4 y) {\n  return max(sign(x - y), 0.0);\n}\n\n\n\nfloat map_6_2(float value, float inMin, float inMax, float outMin, float outMax) {\n  return outMin + (outMax - outMin) * (value - inMin) / (inMax - inMin);\n}\n\nvec2 map_6_2(vec2 value, vec2 inMin, vec2 inMax, vec2 outMin, vec2 outMax) {\n  return outMin + (outMax - outMin) * (value - inMin) / (inMax - inMin);\n}\n\nvec3 map_6_2(vec3 value, vec3 inMin, vec3 inMax, vec3 outMin, vec3 outMax) {\n  return outMin + (outMax - outMin) * (value - inMin) / (inMax - inMin);\n}\n\nvec4 map_6_2(vec4 value, vec4 inMin, vec4 inMax, vec4 outMin, vec4 outMax) {\n  return outMin + (outMax - outMin) * (value - inMin) / (inMax - inMin);\n}\n\n\n\nfloat qinticIn_3_3(float t) {\n  return pow(t, 5.0);\n}\n\n\n\nfloat qinticOut_4_4(float t) {\n  return 1.0 - (pow(t - 1.0, 5.0));\n}\n\n\n\nfloat hue2rgb_5_5(float f1, float f2, float hue) {\n    if (hue < 0.0)\n        hue += 1.0;\n    else if (hue > 1.0)\n        hue -= 1.0;\n    float res;\n    if ((6.0 * hue) < 1.0)\n        res = f1 + (f2 - f1) * 6.0 * hue;\n    else if ((2.0 * hue) < 1.0)\n        res = f2;\n    else if ((3.0 * hue) < 2.0)\n        res = f1 + (f2 - f1) * ((2.0 / 3.0) - hue) * 6.0;\n    else\n        res = f1;\n    return res;\n}\n\nvec3 hsl2rgb_5_6(vec3 hsl) {\n    vec3 rgb;\n    \n    if (hsl.y == 0.0) {\n        rgb = vec3(hsl.z); // Luminance\n    } else {\n        float f2;\n        \n        if (hsl.z < 0.5)\n            f2 = hsl.z * (1.0 + hsl.y);\n        else\n            f2 = hsl.z + hsl.y - hsl.y * hsl.z;\n            \n        float f1 = 2.0 * hsl.z - f2;\n        \n        rgb.r = hue2rgb_5_5(f1, f2, hsl.x + (1.0/3.0));\n        rgb.g = hue2rgb_5_5(f1, f2, hsl.x);\n        rgb.b = hue2rgb_5_5(f1, f2, hsl.x - (1.0/3.0));\n    }   \n    return rgb;\n}\n\nvec3 hsl2rgb_5_6(float h, float s, float l) {\n    return hsl2rgb_5_6(vec3(h, s, l));\n}\n\n\n\nvoid main() {\n  float t = sin(time * 0.01) * 0.5 + 0.5;\n  vec3 color = hsl2rgb_5_6(vec3(t * 0.2 + 0.4 + vNoise * 0.1, 0.9 - (vR * 0.2), clamp(0.8 - (vNoise * 0.6), 0.0, 0.6)));\n  gl_FragColor = vec4(color, 1.0);\n}\n"
